-- 装备箱武藏排行
local WzRankLayer = class("WzRankLayer", LayerEx)


local SortList = {}

function WzRankLayer:create()
	local p = WzRankLayer:new()
	p:init()
	return p
end

function WzRankLayer:init()
...
function WzRankLayer:initRank(tab)
...
function WzRankLayer:clonePanel(panel)
	if panel == nil then
		return
	end
...
function WzRankLayer:createPanel(userData)
...
function WzRankLayer:getPageData(num)
	if num == nil then
		num = 0
	end
...
function WzRankLayer:setButtonPageUp()
...
function WzRankLayer:setButtonPageDdown()
...
function WzRankLayer:setPageNum(num)
	if not num then
		num = 1
	end
...
function WzRankLayer:showLayer()
	HttpManagerEx:getRankingListByOne(8, function(status, errcode, errmsg, data)
		if status == 200 then
			if errcode == 0 then
				if data then
...
function WzRankLayer:setButton()
	self.Button_quit:releaseFunc(function()
		PopupLayerController:hideLayer("WzRankLayer", function(layer)
			self:hide()
		end,0)
	end)
end

Helper:classDefNodeGetInstance(WzRankLayer)

return WzRankLayer00000000000