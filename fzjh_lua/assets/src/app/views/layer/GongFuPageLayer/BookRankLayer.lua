-- 藏书排行
local BookRankLayer = class("BookRankLayer", LayerEx)


local SortList = {}

function BookRankLayer:create()
	local p = BookRankLayer:new()
	p:init()
	return p
end

function BookRankLayer:init()
...
function BookRankLayer:initRank(tab)
...
function BookRankLayer:clonePanel(panel)
	if panel == nil then
		return
	end
...
function BookRankLayer:createPanel(userData)
	...
	panel.Text_menpai:setString(chenghao)
...
function BookRankLayer:getPageData(num)
	if num == nil then
		num = 0
	end
...
function BookRankLayer:setButtonPageUp()
...
function BookRankLayer:setButtonPageDdown()
...
function BookRankLayer:setPageNum(num)
	if not num then
		num = 1
	end
...
function BookRankLayer:showLayer()
	HttpManagerEx:getRankingListByOne(5, function(status, errcode, errmsg, data)
		if status == 200 then
			if errcode == 0 then
				if data then
...
function BookRankLayer:setButton()
	self.Button_quit:releaseFunc(function()
		PopupLayerController:hideLayer("BookRankLayer", function(layer)
			self:hide()
		end, 0)
	end)
end

Helper:classDefNodeGetInstance(BookRankLayer)

return BookRankLayer0