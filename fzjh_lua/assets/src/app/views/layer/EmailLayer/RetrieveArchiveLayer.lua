...
--转移设备
local RetrieveArchiveLayer = class("RetrieveArchiveLayer", LayerEx)

function RetrieveArchiveLayer:create()
	local p = RetrieveArchiveLayer:new()
	p:init()
	return p
end

function RetrieveArchiveLayer:init()
	local UI = require("Layer/RetrieveArchiveUI.lua").create()["root"]
	UI:addTo(self)
...
function RetrieveArchiveLayer:show(openUIType)
...
function RetrieveArchiveLayer:hide()
	PopupLayerController:hideLayer("RetrieveArchiveLayer", function(layer)
		self:setVisible(false)
	end)
end

function RetrieveArchiveLayer:setBack()
...
function RetrieveArchiveLayer:setEditBox()
	self.editBoxMailStr = ""
	self.editBoxCodesStr = ""
...
function RetrieveArchiveLayer:setButtonCodes()
	--获取验证码
	self.Button_Send:releaseFunc(function()
		...
		--判断邮件是否发送成功
		Account:sendEmail("登录", self.editBoxMailStr,
...
function RetrieveArchiveLayer:setButtonTransfer()
	--确认绑定
	self.Button_Bind:releaseFunc(function()
		...
		--判断验证码是否正确
		Account:loginDevice(self.editBoxMailStr, tonumber(self.editBoxCodesStr),
		function(eventName, errmsg)
			if eventName == "登录成功" then
				self.sendTime = 0
				User:resetRole()
				Account:downloadUserData(
				function(eventName, roleData, errmsg)
					print("type(roleData) = "..type(roleData))
					if eventName == "有存档" then
						DataBase:setRoleData(roleData)
						-- DataBase:setData("RoleData", roleData)
						Game:restart(
						function()
...
function RetrieveArchiveLayer:update( ft )
	if self.isSend == true then
...
function RetrieveArchiveLayer:downloadUserData()
...
--转移设备成功后的操作
function RetrieveArchiveLayer:transferSuccess()
...
Helper:classDefNodeGetInstance(RetrieveArchiveLayer)

return RetrieveArchiveLayer
00