...
--转移设备
local LogoutLayerInMenu = class("LogoutLayerInMenu", LayerEx)

-----------------------------------------------------------------------------------------------------------
-- @author TangJian
-- @desc 弹出退出登录
function LogoutLayerInMenu.pop()
	PopupLayerController:showLayer("LogoutLayerInMenu", function(layer)
		layer:show()
	end)
end

function LogoutLayerInMenu:create()
	local p = LogoutLayerInMenu:new()
	p:init()
	return p
end

function LogoutLayerInMenu:init()
...
function LogoutLayerInMenu:show(openUIType)
...
function LogoutLayerInMenu:hide()
	PopupLayerController:hideLayer("LogoutLayerInMenu", function(layer)
		self:setVisible(false)
	end)
end

function LogoutLayerInMenu:setBack()
...
function LogoutLayerInMenu:setEditBox()
	self.editBoxMailStr = ""
	self.editBoxCodesStr = ""
...
function LogoutLayerInMenu:setButtonCodes()
	--获取验证码
...
function LogoutLayerInMenu:setButtonTransfer()
	--确认绑定
	self.Button_Bind:releaseFunc(function()
		...
		--判断验证码是否正确
		Account:logoutDeviceWithoutSave(self.editBoxMailStr, tonumber(self.editBoxCodesStr),
...
function LogoutLayerInMenu:update( ft )
	if self.isSend == true then
...
function LogoutLayerInMenu:downloadUserData()
...
--转移设备成功后的操作
function LogoutLayerInMenu:transferSuccess()
...
Helper:classDefNodeGetInstance(LogoutLayerInMenu)

return LogoutLayerInMenu
0