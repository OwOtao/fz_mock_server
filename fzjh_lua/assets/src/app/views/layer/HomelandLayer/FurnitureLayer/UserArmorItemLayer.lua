local UserArmorItemLayer = class("UserArmorItemLayer", cc.Layer)
...
function UserArmorItemLayer:create()
    local p = UserArmorItemLayer:new()
    p:init()
    return p
end

function UserArmorItemLayer:init()
...
--@collectSocre:武藏积分
--@userId: 对方的mid
function UserArmorItemLayer:showLayer(itemList, collectSocre, userId)
    self.Text_desc:move(cc.p(540, 1650))
    self.Text_desc:setString(HomeLandDesc:getYiGuiDesc(ShenBingDesc:getCollectScore(collectSocre)))
    self.itemIdList = {}
...
function UserArmorItemLayer:initList(itemList, userId)
...
function UserArmorItemLayer:showItemList()
...
function UserArmorItemLayer:clickOneItem(item, itemAttr)
...
function UserArmorItemLayer:getItemPanel()
...
-- 物品描述显示
function UserArmorItemLayer:itemDescShow(anim)
...
-- 物品描述隐藏
function UserArmorItemLayer:itemDescHide(anim)
