local AnZhuoLeiJiChongZhiLayer = class("AnZhuoLeiJiChongZhiLayer", LayerEx)
function AnZhuoLeiJiChongZhiLayer:create()
	local p = AnZhuoLeiJiChongZhiLayer:new()
	p:init()
	return p
end
local rewardList = {}
...
local panelList = {}
function AnZhuoLeiJiChongZhiLayer:init()
...
function AnZhuoLeiJiChongZhiLayer:showLayer(tab)
	local layer = self:getInstance()
	-- self:show()
	-- layer:getGiftList()
	layer:getNultiFestivalGiftList(tab)
end
function AnZhuoLeiJiChongZhiLayer:getNultiFestivalGiftList(tab)--初始化界面
...
function AnZhuoLeiJiChongZhiLayer:setActionTime(actionId)--设置活动时间
...
function AnZhuoLeiJiChongZhiLayer:setLoadingBar(data)--设置充值进度条
...
function AnZhuoLeiJiChongZhiLayer:createGiftPanel(data)--初始化奖励界面
...
function AnZhuoLeiJiChongZhiLayer:setButtonTotalPrize(data)--初始化领取按钮
	local list, shopping_money,giftList = data.list, data.shopping_money,data.gift_list
	if MapIsEmpty(list) == true or shopping_money == nil then
		return
	end
...
function AnZhuoLeiJiChongZhiLayer:createPanelItem(list,maxMoney)--设置奖励panel的位置
	if list == nil then
...
function AnZhuoLeiJiChongZhiLayer:setPanelDetail(panel,list)
...
function AnZhuoLeiJiChongZhiLayer:helpSetPanelDetail(panel,list)
...
function AnZhuoLeiJiChongZhiLayer:createChoosePanel(panel,list,rPanel)
...
function AnZhuoLeiJiChongZhiLayer:dealChoosePanel(panel,list)--初始化选择奖励
	if panel == nil or list == nil then
		return
	end
	panel.Text_num:setString("X"..list.count)
...
function AnZhuoLeiJiChongZhiLayer:setGetGiftButton(rewards,list)
	print("奖励列表详情信息")
...
function AnZhuoLeiJiChongZhiLayer:getOrderIdWithTypeId(typeid, key,rewards)--获取订单的orderId
	Order:getOrderIdFromWeb(2, {key = key}, function(orderId)
		self:receiveNultiFestivalGift(key, orderId,rewards)
	end)
end
function AnZhuoLeiJiChongZhiLayer:receiveNultiFestivalGift(key, order_id,rewards)--领取奖励
	Helper:print_lua_table(rewards)
	local tab = rewards[6]
	if tab == nil then 
...
function AnZhuoLeiJiChongZhiLayer:getReward(key, order_id,itemId,rewards)
...
function AnZhuoLeiJiChongZhiLayer:setButtonCloseAndGoToPay()
...
function AnZhuoLeiJiChongZhiLayer:helpSetGiftButton(list)
	if list == nil then
...
function AnZhuoLeiJiChongZhiLayer:getChooseList(id,list)--
...
function AnZhuoLeiJiChongZhiLayer:getRandomList(list)--随机获取奖励
	if list == nil then
		return
	end
	return list[math.random(1,#list)]
end
function AnZhuoLeiJiChongZhiLayer:getMinMoney(list)--获取最小充值奖励金额
...
function AnZhuoLeiJiChongZhiLayer:getMaxMoney(list)--获取最大充值奖励金额
	if list == nil or type(list) ~= "table"  then
		return
	end
	local max = 0
...
function AnZhuoLeiJiChongZhiLayer:getNowMoney(list)--获取当前可领取奖励档位金额
	if list == nil  or type(list) ~= "table" then
		return
	end
	local tab = self:serializeTable(list)
...
function AnZhuoLeiJiChongZhiLayer:serializeTable(list)--序列化一个表
	 local tab = {}
	 local i = 1
	 for k,v in pairs(list) do 
...
function AnZhuoLeiJiChongZhiLayer:setNodeTouchEnable(node,loop)--设置节点是否可以触摸
	if node == nil then
		return
	end
	if loop ~= nil then
...
function AnZhuoLeiJiChongZhiLayer:setButtonEnabled(button,loop)--设置按钮是否可以点击
	if button == nil then
		return
	end
	if loop ~= nil then
...
function AnZhuoLeiJiChongZhiLayer:setNodeVisibled(node,loop)
	if node == nil then
		return
	end
	if loop ~= nil then
...
function AnZhuoLeiJiChongZhiLayer:setChooseNodeVisibled(loop)
	if loop ~= nil then
...
function AnZhuoLeiJiChongZhiLayer:initGiftTable(tab)
	local list = tab
...
Helper:classDefNodeGetInstance(AnZhuoLeiJiChongZhiLayer)

return AnZhuoLeiJiChongZhiLayer0000