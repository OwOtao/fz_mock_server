--[[
    @desc: 赛龙舟活动页面，仅仅是复制了放风筝活动页面（app.views.layer.ActionLayer.FlyKiteLayer），并修改了几个文字，放风筝替换为赛龙舟
    author: HanTao
    time: 2019-06-03 16:30:52
]]
local PlayDragonBoatLayer = class("PlayDragonBoatLayer", LayerEx)
function PlayDragonBoatLayer:create()
	local p = PlayDragonBoatLayer:new()
	p:init()
	return p
end
...
function PlayDragonBoatLayer:initUI()
...
function PlayDragonBoatLayer:setDscText()
	 self.Text_1:setString("\n活动时间：6月25日0时 — 7月8日23时59分\n活动期间内前往平安小镇寻找“邹笙”可参与龙舟大赛，每日可参与两次，取成绩最好的一次进行保存及累计，结束活动后根据“门派”与“个人”排行来领取奖励。传承积分不保留。\n领奖时间：7月9日12时 — 7月10日23时59分")
	-- self.Text_1_0:setString("舟活动，完成活动可获取赛龙舟活动积分，在活动结")
	-- self.Text_1_1:setString("束后根据玩家门派的最高积分排名来获取奖励。")
end
function PlayDragonBoatLayer:initTable(tab)
	for i=1,table.getn(rewardList) do 
...
function PlayDragonBoatLayer:clonePanel(panel)
	if panel == nil then
		return
	end
...
function PlayDragonBoatLayer:setBack()
	self.Panel_back:releaseFunc(function()
		-- self:hide()
		-- self:destroyInstance() -- add by XiaoZhiWei 2017/09/11 15:05:38 弹出类窗口,隐藏时删除自身
			PopupLayerController:hideLayer("PlayDragonBoatLayer",function(layer)
				layer:hide()
			end)
	end)
end
function PlayDragonBoatLayer:buttonToRankList()
	self.Text_title:setString("龙舟大赛")
	local str = "本次活动传承后将继承龙舟大赛积分\n本次活动结束时将根据积分排行榜发放门派、个人奖励"
...
--春分积分的调用(显示春分积分活动)
function PlayDragonBoatLayer:getAllTypePoint()
...
Helper:classDefNodeGetInstance(PlayDragonBoatLayer)
return PlayDragonBoatLayer
0000000000000000