local KuangHuanLayer = class("KuangHuanLayer", LayerEx)

function KuangHuanLayer:create()
	local p = KuangHuanLayer:new()
	p:init()
	return p
end

function KuangHuanLayer:init()
...
function KuangHuanLayer:setBack()
...
-- 更新时间
function KuangHuanLayer:UpdateTimeAndCount(dt)
...
-- 获取限时礼包状态
function KuangHuanLayer:getState(actionId)
        ...
        if status == 200 then
            if errcode == 0 then
            	if type(data) == "table" and data.is_open == 1 and data.status == 1 then
        			...
        			local extraList = Helper:getDef(data.extra,{})
        			limit = Helper:getDef(tonumber(extraList.limitTimes),20)
        			self.surplusCount = Helper:getDef(tonumber(extraList.limitTimes),20) - Helper:getDef(tonumber(extraList.buyTimes),0)
        			...
        			self.Text_title:setString(Helper:getDef(data.name,"江湖狂欢礼包"))
        			self.limit = limit
        			self:setButton(actionId)
...
-- 初始化礼包道具列表
function KuangHuanLayer:initItemList()
...
function KuangHuanLayer:createPanel(index, item1, item2)
	...
	local function initPanel(itemData, panel)
		...
		local ImagePath =
		{
			...
			zuoyouhubo2 = "Image/UI/StoreUI/juhuajiu100.png",
			xinggongsan = "Image/UI/StoreUI/shenlishan.png",
			jianghuzhaoshimiji1 = "Image/UI/StoreUI/shuxiang.png"
		}
...
function KuangHuanLayer:setButton(actionId)
	self.Button_confirm:releaseFunc(function()
		...
		PopYuanBaoBuyItemLayer("jianghukuanghuan1", function(eventType)
			print("-------------------------:",eventType)
...
Helper:classDefNodeGetInstance(KuangHuanLayer)

return KuangHuanLayer000000000