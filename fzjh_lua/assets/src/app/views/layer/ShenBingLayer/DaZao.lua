local  DaZao = class("DaZao",cc.Layer)

function DaZao:create()
	local p = DaZao.new()
	p:init()
	return p
end
...
function DaZao:init()
	self._UI = require("Layer/ShenBing/DaZaoUI.lua").create()['root']
	self._UI:addTo(self)
	Helper:convertUI(self)

	self.colorid = 1
	--点击背景打造界面隐藏
	self.Layer_back:releaseFunc(function (ref,eventType)
		PopupLayerController:hideLayer("DaZao", function(layer)
			self:hide()
		end)
	end)

	--设置颜色的ZOrder,不然会被exdio遮挡住
	self.Text_Weapon_color:setLocalZOrder(10)
	self.Button_randomName:setLocalZOrder(10)
	--测试用
	User:getRole().shenBingweapon.name = nil
	User:getRole().shenBingweapon.colorname = nil 
	...
	--进入这个界面。设置武器类型的显示
	self.Text_Weapon_type:setString(User:getRole().shenBingweapon.type.."类")
	-- --确定后获得武器的描述
	-- self:setDesc(list.desc,list.out,list.weapon_in)
	self:show()
end
...
-- C+A+B+D        4
-- C+D            5
function DaZao:getRandomName(Type,num)
	local A, B, C,name,canNot= {}, {}, {} ,{},{}
	A = {"天","地","玄","皇","宇","宙","洪","荒","日","月","辰","宿","岁","阳","云","雨","金","木","水","火","土","寒","玉","魔","巨","夜","光","珍","重","海","淡","羽","鱼","龙","师","帝","文","国","七","唐","民","罪","周","汤","朝","道","问","平","章","伏","凤","万","良","知","墨","靖","景","贤","圣","名","宝","空","君","庆","严","松","川","渊","定","诚","荣","政","棠","乐","易","怀","守","真","雁","州","武","古","银","琴","忠","越","义","宋","山","冷","霜","广","鸿","冰"} 
...
function DaZao:show(ControllLayer)
	-- MainControllLayer = ControllLayer
	self:setVisible(true)
	self:Input()
	self:TextColor()
	self:choiceWeaponType()
end

--武器类别的选择
function DaZao:choiceWeaponType()
	self.Text_Weapon_type:setButtonType(WIDGET_TOUCH_VOICE_TYPE_SMALLBUTTON)
	self.Text_Weapon_type:releaseFunc(function()
		if PRINT_MODE == 1 then 
			print("****************************")
		end
	end)
end
function DaZao:ShenBingDesc()
	--初始化的时候，如果神兵没有升级过，就是原始描述，根据类别设置一个最原始的描述
	local role = User:getRole()
	if role.shenBingweapon.type == "剑" then
		role.shenBingweapon.dsc = "这是一把剑，重约五斤，尚未开刃，似乎一碰即碎，无甚奇异。"
	end
	if role.shenBingweapon.type == "刀" then
		role.shenBingweapon.dsc = "这是一把刀，重约七斤，尚未开刃，似乎一碰即碎，无甚奇异。"
	end		
	if role.shenBingweapon.type == "棍" then
		role.shenBingweapon.dsc = "这是一根棍，重约七斤，看上去极易折断无甚奇异。"
	end
	if role.shenBingweapon.type == "鞭" then
		role.shenBingweapon.dsc = "这是一跟鞭，重约两斤，鞭身似乎十分松散，一拉即断，无甚奇异。"
	end

end
function DaZao:Input()
	local role = User:getRole()
	 -- 初始化变量
	self._editBoxString = ""
	...
	local shenBingweapon = role:getAttr("shenBingweapon")
	...
	--第一次设置名字
	if shenBingweapon.name == nil or shenBingweapon.name == "" then
		editBox:setText(nil)
		editBox:setText("请输入名字")
		-- self.TextField_WeaponName:setString("请输入名字")
		editBox:setFontColor(colorType[1].color)
	else
		--将玩家武器名和颜色显示出来（已经设置过名字，不能更改）
		self.editBox:setTouchEnabled(false)
		self.editBox:setText(shenBingweapon.name)
		local color
		for i=1 ,#colorType  do
			if colorType[i].name == shenBingweapon.colorname then
				color = colorType[i].color
				-- Helper:print_lua_table(color)
				break
			end
		end
		self.editBox:setFontColor(color)
		--武器确定后，颜色也不能更改
		self.Text_Weapon_color:setColor(color)
		self.Text_Weapon_color:setTouchEnabled(false)--设置不可触摸
	...
	self.Button_randomName:releaseFunc(function()
		...
		if self._isEditing == false then
    		local role = User:getRole()
	    	editBox:setText(self:getRandomName(role.shenBingweapon.type,num))
	    	self._editBoxString = editBox:getText()
		end
	end)
	...
	self.Button_Yes:releaseFunc(function()
	    ...
	    -- self.TextField_WeaponName:setString(self._editBoxString)
	    if self:checkNameCanUse(self._editBoxString,shenBingweapon.type) then
	    ...
	    if shenBingweapon.name == nil or shenBingweapon.name == "" then
	    	--初始化神兵属性
			shenBingweapon.id ="神兵"  --设置一个唯一的id  方便在物品里使用
			shenBingweapon.onlyId = User:getRoleAttr("userid")..role:getItemOnlyId()
		    shenBingweapon.name = self.editBox:getText()
		    -- shenBingweapon.type = "刀"--到剑棍鞭中的一种
		    shenBingweapon.status = "2" --锻造的阶段
		    shenBingweapon.unit = "一把"			-- 单位
			...
			shenBingweapon.equipPart = "weapon" --装的时候用的属性
			shenBingweapon.lv = 1 --神兵初始化等级为1
			shenBingweapon.gold_level = 0
			shenBingweapon.neili_level = 0
			shenBingweapon.loadingBarPersent = 0
		    shenBingweapon.damage = 11 --初始化伤害值10			
		    ...
		    if self.colorid == 1 then
		    	shenBingweapon.colorname = colorType[self.colorid].name
			    shenBingweapon.color = colorType[self.colorid].color
				shenBingweapon.colorid = self.colorid

		    else
			    shenBingweapon.colorname = colorType[self.colorid-1].name
			    shenBingweapon.color = colorType[self.colorid-1].color
				shenBingweapon.colorid = self.colorid-1
			end

	    	-- 设置名字颜色
	    	shenBingweapon.name = tostring(shenBingweapon.colorname)..tostring(shenBingweapon.name)
		    ...
		    if role:addItemCount(shenBingweapon.id, 1) then
		    	PopText("成功锻造您的 "..shenBingweapon.name.." 神兵")
		    end
		 ...
		 --    local  Type 
		 --    if shenBingweapon.type == "剑" then Type = 1 end
		 --    if shenBingweapon.type == "刀" then Type = 2 end
		 --    if shenBingweapon.type == "棍" then Type = 3 end
		 --    if shenBingweapon.type == "鞭" then Type = 4 end
		 --    ---向服务器发信息
		 --    -- 需要的参数
			-- local params1=
			-- {
			--  weapon_type = Type,
			--  weapon_name = shenBingweapon.name,
			--  weapon_color = self.colorid - 1 ,
			-- }
			-- --领取
			-- HttpManagerEx:makeWeapon(params1, function(status, errcode, errmsg, data)
			-- 	if status == 200 then
			-- 		--确定后获得武器的描述
			-- 		self:setDesc(data.desc,data.out,data.weapon_in)
			-- 		return true
			-- 	else
			-- 		PopText(string(errmsg))
			-- 	end
			-- end,IS_SHOW_WAITING, HTTP_MANAGER_RETRY_TYPE_RETRY)
			-- HIC只见欧冶子师傅抽出鱼肠剑，在宝剑上随手挥洒，立马就有了[兵器名]龙飞凤舞的细字，直把你看得目瞪口呆。
			
			-- CYN你拿着剑端详一下，宝剑暗纹浮动，真是绝世无双的手艺。
		...
		if PRINT_MODE == 1 then
			print(".........................................................")
			Helper:print_lua_table(role:getItems())
			print(".........................................................")
		end
		--命名完成后，刷新打造中的界面
		if shenBingweapon.status == "2" and shenBingweapon.name ~= nil or shenBingweapon.name ~= "" then
			local shenBingLayer = MainControllLayer:getLayer("ShenBingLayer")
			shenBingLayer.Panel_DuanZaoZhong:setVisible(true)
		--这里处理兵器打造完成后，应该显示的界面
			shenBingLayer.Panel_DuanZaoZhong.Image_myweapon.Text_Weapon1:setString(User:getRole().shenBingweapon.name)
			shenBingLayer.Panel_DuanZaoZhong.Image_myweapon.Text_Weapon1:setColor(User:getRole().shenBingweapon.color)
			shenBingLayer.Panel_DuanZaoZhong.Button_2.Text_button2Name:setString("强化")
			shenBingLayer:print("HIC只见欧冶子师傅抽出鱼肠剑，在宝"..role.shenBingweapon.type.."上随手挥洒，立马就有了"..role.shenBingweapon.colorname..role.shenBingweapon.name.."HIC龙飞凤舞的细字，直把你看得目瞪口呆。")
			shenBingLayer:print("CYN成了。拿去吧！说完便把"..role.shenBingweapon.type.."递了过来")
			shenBingLayer:print("CYN你拿着"..role.shenBingweapon.type.."端详一下，宝"..role.shenBingweapon.type.."暗纹浮动，真是绝世无双的手艺。")
		elseif shenBingweapon.status == "1" and shenBingweapon.name == nil or shenBingweapon.name == "" then
			shenBingLayer.Panel_DuanZaoZhong.Image_myweapon.Text_Weapon1:setString("我的剑")
			shenBingLayer.Panel_DuanZaoZhong.Button_2.Text_button2Name:setString("领取")
		end
...
--接受文字输入
-- function DaZao:TextFieldWeaponName()
-- 	self.EditBoxArea:setButtonType(WIDGET_TOUCH_VOICE_TYPE_SMALLBUTTON)
-- 	self.EditBoxArea:releaseFunc(function()
-- 		self:Input()
-- 	end)
-- end
...
--触摸切换颜色
function DaZao:TextColor()
	Helper:print_lua_table(colorType[self.colorid])
	self.Text_Weapon_color:setButtonType(WIDGET_TOUCH_VOICE_TYPE_SMALLBUTTON)
	self.Text_Weapon_color:releaseFunc(function()
		--如果是这里加一，则在取名字后颜色就变成
		if self.colorid ==#colorType+1 then
			self.colorid = 1
		end
		if PRINT_MODE == 1 then
			print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
		...
		self.Text_Weapon_color:setColor(colorType[self.colorid].color)
		self.editBox:setFontColor(colorType[self.colorid].color)
...
--打造界面（领取界面的描述添加）
function DaZao:setDesc(strAppearance,strBa,strHui)
...
Helper:classDefNodeGetInstance(DaZao)
return  DaZao00000000000000