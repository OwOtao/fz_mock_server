...
local ZhaoImprovedPoolPresenter = {}

function ZhaoImprovedPoolPresenter:create(IZhaoImprovedPoolOutput,selfCreatedSkillSystem,zhaoIndex)
    local p = inherit({}, ZhaoImprovedPoolPresenter)
    p:init(IZhaoImprovedPoolOutput,selfCreatedSkillSystem,zhaoIndex)
    return p
end

function ZhaoImprovedPoolPresenter:init(IZhaoImprovedPoolOutput,selfCreatedSkillSystem,zhaoIndex)
...
function ZhaoImprovedPoolPresenter:showLayer(propList,canUseItem,callback,skillId)
...
function ZhaoImprovedPoolPresenter:initTabView()
...
function ZhaoImprovedPoolPresenter:filtraList(type,list)
    local newList = {}
...
function ZhaoImprovedPoolPresenter:createListView(index)
    local list = {}
    local retData = {}
    self.hidePreBg = nil
...
function ZhaoImprovedPoolPresenter:showItemDetailLayer(item,row)
        ...
        function(layer)
            ...
            if self._canUseItem then
                layer:setRightBtn(
                    "使 用",
                    function()
						if self.__useTime and GetTime() - self.__useTime < 0.5 then
							return
						end

                        Audio:playEffect("xiaoAnNiu")

                        local skillId = self._selfCreatedSkillSystem:getSelfCreatingSkill():getId()
                        self._selfCreatedSkillSystem:useImproveProp(item.id,self._zhaoIndex,skillId,function(isUse,data)
							if isUse then
...
function ZhaoImprovedPoolPresenter:setTitleImageShow(panel)
    self._IZhaoImprovedPoolOutput:setTitleImageShow(panel)
end

function ZhaoImprovedPoolPresenter:setImageBack()
...
function ZhaoImprovedPoolPresenter:setPanelBack()
...
function ZhaoImprovedPoolPresenter:showTextTital()
    self._IZhaoImprovedPoolOutput:setTextTital(false)
end

function ZhaoImprovedPoolPresenter:showTextDsc()
    local text = "请选择你要使用的道具"
    self._IZhaoImprovedPoolOutput:setTextDsc(text,true)
end

function ZhaoImprovedPoolPresenter:showTextDesc()
    local text = "\n点击上方感叹号可查看关于改良池玩法介绍。"
    self._IZhaoImprovedPoolOutput:setTextDesc(false)
end

function ZhaoImprovedPoolPresenter:setTipDsc()
    local text = "完成历练任务有几率获得卷轴。\n卷轴可用来对当前招式进行优化与改良。\n传承将会保留已拥有的道具"
    self._IZhaoImprovedPoolOutput:setTipDsc(false)
end

function ZhaoImprovedPoolPresenter:hideLayer()
    self._IZhaoImprovedPoolOutput:hideLayer()
end

isImplement(ZhaoImprovedPoolPresenter, IZhaoImprovedPoolPresenterInput)
return ZhaoImprovedPoolPresenter
000000000000