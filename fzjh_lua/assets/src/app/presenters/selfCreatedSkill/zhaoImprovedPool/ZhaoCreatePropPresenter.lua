...
local ZhaoCreatePropPresenter = {}

function ZhaoCreatePropPresenter:create(IZhaoImprovedPoolOutput,selfCreatedSkillSystem,zhaoIndex)
    local p = inherit({}, ZhaoCreatePropPresenter)
    p:init(IZhaoImprovedPoolOutput,selfCreatedSkillSystem,zhaoIndex)
    return p
end

function ZhaoCreatePropPresenter:init(IZhaoImprovedPoolOutput,selfCreatedSkillSystem,zhaoIndex)
...
function ZhaoCreatePropPresenter:showLayer(propList,canUseItem,callback,skillId)
...
function ZhaoCreatePropPresenter:initTabView()
    self._titleTable = {
...
function ZhaoCreatePropPresenter:createListView(index)
    local list = {}
    local retData = {}
    self.hidePreBg = nil
    ...
    if not MapIsEmpty(list) then
        ...
        self._IZhaoImprovedPoolOutput:setTextEmpty(true,"你的道具池空空如也")
    end
...
function ZhaoCreatePropPresenter:showItemDetailLayer(item,row)
        ...
        function(layer)
            ...
            if self._canUseItem then
                layer:setRightBtn(
                    ...
                    function()
                        Audio:playEffect("xiaoAnNiu")
                        self._selfCreatedSkillSystem:useCreateProp(item.id,function()
                            self._selfCreatedSkillSystem:getSkillCreator():setPropId(item.id)
...
function ZhaoCreatePropPresenter:setTitleImageShow(panel)
    self._IZhaoImprovedPoolOutput:setTitleImageShow(panel)
end

function ZhaoCreatePropPresenter:setImageBack()
...
function ZhaoCreatePropPresenter:setPanelBack()
...
function ZhaoCreatePropPresenter:showTextTital()
    self._IZhaoImprovedPoolOutput:setTextTital(false)
end

function ZhaoCreatePropPresenter:showTextDsc()
    local text = "请选择你要使用的道具"
    self._IZhaoImprovedPoolOutput:setTextDsc(text,true)
end

function ZhaoCreatePropPresenter:showTextDesc()
    self._IZhaoImprovedPoolOutput:setTextDesc(false)
end

function ZhaoCreatePropPresenter:setTipDsc()
    self._IZhaoImprovedPoolOutput:setTipDsc(false)
end

function ZhaoCreatePropPresenter:hideLayer()
    self._IZhaoImprovedPoolOutput:hideLayer()
end

isImplement(ZhaoCreatePropPresenter, IZhaoImprovedPoolPresenterInput)
return ZhaoCreatePropPresenter
0000000000000