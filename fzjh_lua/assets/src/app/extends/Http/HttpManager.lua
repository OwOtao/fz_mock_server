...
local HttpManager = require("app.extends.Http.Http")
...
--@desc: 江湖秘宝活动充值
--@author:LvBin
--@time:2024-08-05 16:30:30
--@key:
--@activityId: 活动id
--@dataVer: 行为系统版本号
--@currencyVersion:货币版本
--@callback:
--@isNeedWait:
--@retryType: 
--@return
function HttpManager:checkJHMBActionPaySign(key, activityId,dataVer, currencyVersion, callback, isNeedWait, retryType)
    if key == nil or activityId == nil then
        return
    end
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "check_pay_sign", {key = key, activity_id = activityId,dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
-- 获取服务器角色相关数据
function HttpManager:getGameUserInfo(noticeId, currencyVersion,callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_game_user_info_2", {notice_version = noticeId, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType, NEED_ENCRYPT)
end
...
--@roleData: 传承后的角色数据
	...
	--@retryType: 
--@return
function HttpManager:inherit(roleData, userId, dataVer, currencyVersion, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "finishNextRoleTask", {roleData = {roleData},dataVer = dataVer, currencyVersion = currencyVersion}, {userid = userId}, callback, isNeedWait, retryType, NEED_ENCRYPT)
end
...
--获取是否有未查看的活动
function HttpManager:getEventList(type, currencyVersion, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_activity_list", {type = type, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--领取消耗奖励
function HttpManager:receiveYuanBaoPlanGift(rid, isEmail, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "receive_yuanbao_plan_gift", {rid = rid, isEmail = isEmail, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
function HttpManager:viewCurrencyByType(currency_type, currencyVersion,callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "view_currency_by_type", {currency_type = currency_type, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--江湖珍品阁抽奖
function HttpManager:doZhenPinGeLottery(activity_id, times, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "do_zhenpinge_lottery", {times = times, activity_id = activity_id, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end

--江湖珍品阁兑换
function HttpManager:exchangeZhenPinGeGoods(activity_id, id, num, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "exchange_zhenpinge_goods", {id = id, activity_id = activity_id, dataVer = dataVer, currencyVersion = currencyVersion, num = num}, nil, callback, isNeedWait, retryType)
end
...
-- 使用创作道具
function HttpManager:useCreateProp(propId, userLv, skillDataId, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "use_create_prop", {prop = propId, lv = userLv, skillId = skillDataId}, nil, callback, isNeedWait, retryType)
end

-- 使用改良道具
function HttpManager:useImproveProp(propId, userLv, zhaoIndex, skillDataId, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "use_improve_prop", {prop = propId, lv = userLv, zhaoIndex = zhaoIndex, skillId = skillDataId}, nil, callback, isNeedWait, retryType)
end
...
--获取每日任务活动对应奖励
function HttpManager:getDailyTaskReward(rid, isEmail, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_daily_task_reward", {rid = rid, isEmail = isEmail, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
--添加每日活动积分
...
--获取登录活动奖励
function HttpManager:getLoginReward(activityId, rid, isEmail, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_login_reward", {activityId = activityId, rid = rid, isEmail = isEmail, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
function HttpManager:getSpendReward(actionId, rid, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_spend_reward", {activity_id = actionId, rid = rid, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@desc 领取邮件奖励
function HttpManager:getEmailReward(id, familyId, retrievables, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_email_reward", {id = id, familyId = familyId, retrievables = retrievables,dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@desc 领取所有邮件奖励
function HttpManager:getAllEmailReward(familyId, retrievables, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_all_email_rewards", {familyId = familyId, retrievables = retrievables, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@desc 面具升级
function HttpManager:maskUpgrade(params, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "mask_upgrade", {params = params, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--江湖夺宝领奖
function HttpManager:lotteryTreasureReward(activity_id, rid, is_email, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "lottery_treasure_reward", {activity_id = activity_id, rid = rid, is_email = is_email, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--购买天缘奇盒奖励
function HttpManager:buyLuckBoxGood(activity_id, rid, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "buy_luck_box_good", {activity_id = activity_id, rid = rid, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end

--领取天缘奇盒奖励
function HttpManager:getLuckBoxGood(activity_id, rid, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_luck_box_good", {activity_id = activity_id, rid = rid, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--江湖秘宝特殊礼包购买
function HttpManager:buySpendReward(activity_id, rid, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "buy_spend_reward", {activity_id = activity_id, rid = rid, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--武学突破道具获取
function HttpManager:martialUpgradeAddCurrency(currencyVersion, type, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "martial_upgrade_add_currency", {currencyVersion = currencyVersion, type = type}, nil, callback, isNeedWait, retryType)
end
...
--驱赶年兽接口
--times 驱赶次数
function HttpManager:expelNian(times, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "expel_nian", {times = times, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end

--获取年兽奖励接口
-- rid 奖励id
function HttpManager:getExpelReward(rid, giftId, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_expel_reward", {rid = rid, giftId = giftId, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@desc 领取奖励，并记录
function HttpManager:challengeMapGetAward(mapId, type, award_list, finishType, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "challengemap_award", {currencyVersion = currencyVersion, map_id = mapId, type = type, award_list = award_list, finish_type = finishType}, nil, callback, isNeedWait, retryType)
end
...
--@retryType:
--@return
function HttpManager:getSkillBreakCurrency(currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_martial_upgrade_currency", {currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@retryType:
--@return
function HttpManager:skillBreakThrough(id, skillId, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "martial_upgrade", {id = id, martial_id = skillId, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@retryType:
--@return
function HttpManager:testAddMartialCurrency(currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "test_add_martial_currency", {currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@retryType:
--@return
function HttpManager:zhaoBreakThrough(id, zhaoId, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "zhao_upgrade", {id = id, zhao_id = zhaoId, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@retryType:
--@return
function HttpManager:getZhaoBreakThroughItems(currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_zhao_upgrade_matters", {currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--type  1 获取 2 刷新
--currencyVersion 货币版本
function HttpManager:mattersShopInfo(type, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "matters_shop_info", {type = type, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end

--购买续卷商人商品
--goodsKey 商品id
--currencyVersion 货币版本
function HttpManager:buyMatters(goodsKey, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "buy_matters", {goodsKey = goodsKey, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
function HttpManager:getTrainingTaskList(actionId, taskList, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_training_task_list", {activityId = actionId, taskList = taskList}, nil, callback, isNeedWait, retryType)
end
...
function HttpManager:refreshTrainingTaskList(actionId, taskList, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "refresh_training_task_list", {activityId = actionId, taskList = taskList}, nil, callback, isNeedWait, retryType)
end
...
--[[
    @desc: 领取历练任务奖励
    author:tanqinjian
    time:2025-09-01 10:54:22
    --@activityId:
	--@rid:奖励项id
	--@giftId:奖励id
    --@isEmail: 是否邮件发放 0 否 1是
	--@dataVer:
	--@currencyVersion:
	--@callback:
	--@isNeedWait:
...
function HttpManager:getTrainingTaskReward(activityId, rid, giftId, isEmail, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_training_task_reward", {activityId = activityId, rid = rid, isEmail = isEmail, dataVer = dataVer, giftId = giftId, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@desc: 删除已购买的所有皮肤（测试专用接口）
--@author:Seven
--@time:2025-10-30 20:09:13
function HttpManager:testDeleteAllUiThemes(callback,isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeader(DOMAIN.."test_delete_all_ui_themes",nil,nil,callback,isNeedWait,retryType)
end
...
--驱赶千杯不醉接口
--times 敬酒次数
function HttpManager:toastQian(times, dataVer, callback,isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN.."toast_qian",{times = times, dataVer = dataVer},nil,callback,isNeedWait,retryType)
end

--获取千杯不醉奖励接口
-- rid 奖励id
function HttpManager:getToastReward(rid, giftId, dataVer, newDataVer, callback,isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN.."get_toast_reward",{dataVer = dataVer, rid = rid, giftId = giftId, newDataVer = newDataVer},nil,callback,isNeedWait,retryType)
end
...
function HttpManager:getLianGongData(dataVer, codeVer, callback)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "getLianGongData", {dataVer = dataVer, codeVer = codeVer}, nil, callback, HTTP_MANAGER_RETRY_TYPE_RETRY, true)
end
...
function HttpManager:getXiuLianState(dataVer, codeVer, callback)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "getXiuLianState", {dataVer = dataVer, codeVer = codeVer}, nil, callback, HTTP_MANAGER_RETRY_TYPE_RETRY, true)
end
...
function HttpManager:xiuLianStart(requestId, dataVer, codeVer, actionData, time, callback)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "xiuLianStart", {requestId = requestId, dataVer = dataVer, codeVer = codeVer, actionData = actionData, time = time}, nil, callback, HTTP_MANAGER_RETRY_TYPE_RETRY, true)
end
...
function HttpManager:xiuLianFinish(requestId, dataVer, codeVer, actionData, time, callback)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "xiuLianFinish", {requestId = requestId, dataVer = dataVer, codeVer = codeVer, actionData = actionData, time = time}, nil, callback, HTTP_MANAGER_RETRY_TYPE_RETRY, true)
end
...
function HttpManager:xiuLianUseXingGongSan(requestId, dataVer, codeVer, time, callback)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "xiuLianUseXingGongSan", {requestId = requestId, dataVer = dataVer, codeVer = codeVer, actionData = "", time = time}, nil, callback, HTTP_MANAGER_RETRY_TYPE_RETRY, true)
end
...
function HttpManager:getXiuLianData(dataVer, codeVer, callback)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "getXiuLianData", {dataVer = dataVer, codeVer = codeVer}, nil, callback, HTTP_MANAGER_RETRY_TYPE_RETRY, true)
end
...
function HttpManager:getXinShenValue(dataVer, codeVer, callback)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "getXinShenValue", {dataVer = dataVer, codeVer = codeVer}, nil, callback, HTTP_MANAGER_RETRY_TYPE_RETRY, true)
end
...
--@retryType: 
--@return
function HttpManager:getWareHouseActivityInfo(currencyVersion, callback,isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN.."get_treasury_info",{currencyVersion = currencyVersion},nil,callback,isNeedWait,retryType)
end
...
--@currencyVersion:货币版本
	...
	--@retryType: 
--@return
function HttpManager:WareHouseDrawLucky(indexList, currencyVersion, callback,isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN.."treasury_lottery",{indexList = indexList, currencyVersion = currencyVersion},nil,callback,isNeedWait,retryType)
end
...
--@rewardType: 1,固定进度奖励;2,随机奖池奖励
	...
	--@retryType: 
--@return
function HttpManager:getWareHouseAward(rewardType,awardIds,isEmail,dataVer,currencyVersion,callback,isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN.."get_treasury_reward",{rewardType = rewardType,awardIds = awardIds,isEmail = isEmail,dataVer = dataVer,currencyVersion = currencyVersion},nil,callback,isNeedWait,retryType)
end
...
--@desc: 获取观影堂特权加送活动数据
--@author:LvBin
--@time:2025-08-15 14:42:20
--@callback:
	--@isNeedWait:
	--@retryType: 
--@return
function HttpManager:getViewingHallPrivilegeInfo(callback,isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeader(DOMAIN.."get_viewing_privilege_info",nil,nil,callback,isNeedWait,retryType)
end
...
--@callback:
	...
	--@retryType: 
--@return
function HttpManager:getViewingHallPrivilegeReward(callback,isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeader(DOMAIN.."get_viewing_privilege_reward",nil,nil,callback,isNeedWait,retryType)
end
...
--优化版香囊密阁获取奖励
function HttpManager:exchangeAwardToSachetNew(activity_id, reward_id, is_email, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "exchange_award_to_sachet_new", {activity_id = activity_id, is_email = is_email, reward_id = reward_id, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
function HttpManager:checkEmailWhite(email, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "check_email_white", {email = email}, nil, callback, isNeedWait, retryType)
end
...
--@currencyVersion:货币版本
	...
	--@retryType: 
--@return
function HttpManager:getZhaoPracticeInfo(npcId , mid, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_zhao_practiceInfo", {npcId = npcId,mid = mid, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@zhaoId: 对练的招式id
	...
	--@retryType: 
--@return
function HttpManager:zhaoPractice(npcId , mid, zhaoId, type, price, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "zhao_practice", {npcId = npcId,mid = mid,zhaoId = zhaoId,type = type,price = price, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--领取招财进宝奖励
function HttpManager:getZhaoCaiJinBaoReward(activity_id, id, reward_id, is_email, dataVer, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_zhaocaijinbao_reward", {id = id, activity_id = activity_id, reward_id = reward_id, dataVer = dataVer, is_email = is_email, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--易金圩市
function HttpManager:getCuiLianCaiLiaoStoreList(activity_id, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_cuiLianCaiLiao_store_list", {activityId = activity_id, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@retryType: 
--@return
function HttpManager:donateTeacherBuild(buildTypeId,donateId,donateState,familyId, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "donate_teacherBuild",{currencyVersion = currencyVersion, buildTypeId = buildTypeId,familyId = familyId,donateId = donateId,donateState = donateState}, nil, callback, isNeedWait, retryType)
end
...
--易金圩市购买礼包
function HttpManager:buyCuiLianCaiLiaoStoreGoods(activity_id, id, currencyId, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "buy_cuiLianCaiLiao_store_goods", {activityId = activity_id, id = id, currencyId = currencyId, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end

--易金圩市获取礼包奖励
function HttpManager:getCuiLianCaiLiaoStoreReward(activity_id, id, is_email, dataVer, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_cuiLianCaiLiao_store_reward", {id = id, activityId = activity_id, dataVer = dataVer, isEmail = is_email}, nil, callback, isNeedWait, retryType)
end

--易金圩市兑换积分
function HttpManager:exchangeCuiLianCaiLiaoStoreIntegral(activity_id, id, number, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "exchange_cuiLianCaiLiao_store_integral", {id = id, activityId = activity_id, number = number, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@currencyVersion:
	...
	--@retryType: 
--@return
function HttpManager:getYouXiaMcmrestrictUpgradeCondition(familyId, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_youxia_upgradeCondition", {familyId = familyId, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@currencyVersion:货币版本
	...
	--@retryType: 
--@return
function HttpManager:youxiaUpgradeMcmrestrict(familyId, currencyVersion, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "youxia_upgrade_condition", {familyId = familyId, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--@desc: 获取制作面具信息
--@author:LvBin
--@time:2024-07-04 11:39:22
--@callback:
	--@isNeedWait:
	--@retryType: 
--@return
function HttpManager:getMakeMaskInfo(callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeader(DOMAIN .. "get_makeMask_info", nil, nil, callback, isNeedWait, retryType)
end

function HttpManager:makeRandomMask(cost,grantType,callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "make_randomMask", {cost = cost, grantType = grantType}, nil, callback, isNeedWait, retryType)
end

function HttpManager:makePayMask(id,spCost,grantType,callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "make_payMask", {id = id, spCost = spCost, grantType = grantType}, nil, callback, isNeedWait, retryType)
end

function HttpManager:getPayMakeMaskGiftInfo(callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeader(DOMAIN .. "get_payMask_gift_info", nil, nil, callback, isNeedWait, retryType)
end

function HttpManager:receivePayMaskGift(id, dataVer, currencyVersion, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "receive_payMask_gift", {id = id, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end

-----------------------------------------------------------------------------------------------------------
--测试用例接口,用获取时间接口举例协程阻塞异步和非协程阻塞用法

--不用协程阻塞异步
function HttpManager:testGetTime(callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeaderOrig(DOMAIN .. "get_time", nil, nil, callback, isNeedWait, retryType, NEED_ENCRYPT)
end

--使用协程阻塞异步
function HttpManager:testGetTimeAsync(callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeader(DOMAIN .. "get_time", nil, nil, callback, isNeedWait, retryType, NEED_ENCRYPT)
end

--不用协程阻塞异步,支持自定义请求界面文本
function HttpManager:testGetTimeWaitText(waitText, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeaderAndWaitTextOrig(waitText ,DOMAIN .. "get_time", nil, nil, callback, isNeedWait, retryType, NEED_ENCRYPT)
end

--使用协程阻塞异步,支持自定义请求界面文本
function HttpManager:testGetTimeWaitTextAsync(waitText, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeaderAndWaitText(waitText ,DOMAIN .. "get_time", nil, nil, callback, isNeedWait, retryType, NEED_ENCRYPT)
end

--师门建筑振兴门派详情获取
function HttpManager:getSectRevitalizationInfo(familyId, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_sectRevitalization_info", {familyId = familyId}, nil, callback, isNeedWait, retryType)
end

--师门建筑振兴门派奖励
function HttpManager:getSectRevitalizationReward(familyId, buildTypeId, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_sectRevitalization_reward", {familyId = familyId, buildTypeId = buildTypeId}, nil, callback, isNeedWait, retryType)
end

--师门建筑易物堂商店
function HttpManager:getSectSupportShop(familyId, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_sect_supportShop", {familyId = familyId}, nil, callback, isNeedWait, retryType)
end

--师门建筑易物堂商品兑换
function HttpManager:BuySectSupportGoods(familyId, id, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "buy_sect_supportGoods", {familyId = familyId, id = id}, nil, callback, isNeedWait, retryType)
end

--修复挂机版本号异常接口
function HttpManager:uploadAbnormalHangUpTask(version, taskId, taskStartTime, extraData, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "upload_abnormal_hangUp_task", {version = version, task_id = taskId, taskStartTime = taskStartTime, extra = extraData}, nil, callback, isNeedWait, retryType)
end

--@desc: 获取拳脚商店信息
--@author:LvBin
--@time:2024-10-31 17:48:00
--@activityId:
	--@callback:
	--@isNeedWait:
	--@retryType: 
--@return
function HttpManager:getFistFootShopInfo(callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeader(DOMAIN .. "get_fistFootShop_info", nil, nil, callback, isNeedWait, retryType)
end

--@desc: 设置每日支付元宝数
--@author:LvBin
--@time:2024-11-06 17:00:35
--@dailySelectCost:支付元宝数
	--@callback:
	--@isNeedWait:
	--@retryType: 
--@return
function HttpManager:setFistFootShopDailyCost(dailySelectCost, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "set_fistFootShop_dailyCost", {dailySelectCost = dailySelectCost}, nil, callback, isNeedWait, retryType)
end

function HttpManager:buyFistFootShopDailyGoods(dayId,dataVer, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "buy_fistFootShop_dailyGoods", {dayId = dayId,dataVer = dataVer}, nil, callback, isNeedWait, retryType)
end

function HttpManager:buyFistFootShopSpecialOffer(specialOfferCost,dataVer, callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "buy_fistFootShop_specialOffer", {specialOfferCost = specialOfferCost,dataVer = dataVer}, nil, callback, isNeedWait, retryType)
end
...
function HttpManager:getwxtranstwxplushasGainAfter202509291500(callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeader(DOMAIN .. "check_user_gain_log", "", nil, callback, isNeedWait, retryType)
end

function HttpManager:testSetMeridianPageNum(num,callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "test_set_meridian_talent_page", {num = num}, nil, callback, isNeedWait, retryType)
end

------------------------------------------------------------------------------------------------------
--@desc: 获取隐脉相关资源
--@author:LvBin
--@time:2025-03-04 14:47:26
function HttpManager:getHiddenMeridianInfo(callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeader(DOMAIN .. "get_hidden_meridian_info", nil, nil, callback, isNeedWait, retryType)
end

--@desc: 开始破境
--@author:LvBin
--@time:2025-03-04 14:51:24
--@id: 隐脉图id
--@version: 版本号
--@currencyVersion:货币版本号
function HttpManager:startHiddenMeridianBreakThrough(id,version,currencyVersion,callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "start_break_through_realm", {id = id,version = version, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end

--@desc: 加速破境
--@author:LvBin
--@time:2025-03-04 14:53:41
--@id: 隐脉图id
--@cost: 加速消耗的资源数量
--@version: 版本号
function HttpManager:speedUpHiddenMeridianBreakThrough(id,cost,version,callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "accelerate_break_through_realm", {id = id,cost = cost,version = version}, nil, callback, isNeedWait, retryType)
end

--@desc: 完成破境
--@author:LvBin
--@time:2025-03-04 14:54:48
--@id: 隐脉图id
--@boostingEffect: 余炁效果属性
--@version:版本号
function HttpManager:finishHiddenMeridianBreakThrough(id,boostingEffect,version,callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "finish_break_through_realm", {id = id,boostingEffect = boostingEffect,version = version}, nil, callback, isNeedWait, retryType)
end

--@desc: 取消破境
--@author:LvBin
--@time:2025-03-04 14:57:20
--@id: 隐脉图id
--@version:版本号
function HttpManager:cancelHiddenMeridianBreakThrough(id,version,callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "cancel_break_through_realm", {id = id,version = version}, nil, callback, isNeedWait, retryType)
end
...
--@id: 窍关id
--@version: 版本号
function HttpManager:startAcupointActivate(id,version,callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "start_thoroughfare_vessel", {id = id,version = version}, nil, callback, isNeedWait, retryType)
end
...
--@cost: 加速消耗的资源数量
--@version: 版本号
function HttpManager:speedUpAcupointActivate(id,cost,version,callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "accelerate_thoroughfare_vessel", {id = id,cost = cost,version = version}, nil, callback, isNeedWait, retryType)
end
...
--@id: 窍关id
--@version:版本号
function HttpManager:finishAcupointActivate(id,version,callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "finish_thoroughfare_vessel", {id = id,version = version}, nil, callback, isNeedWait, retryType)
end
...
--@id: 窍关id
--@version:版本号
function HttpManager:cancelAcupointActivate(id,version,callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "cancel_thoroughfare_vessel", {id = id,version = version}, nil, callback, isNeedWait, retryType)
end

--@desc: 解锁玄络buff
--@author:LvBin
--@time:2025-03-04 15:02:40
--@id: 玄络buffId
--@version: 版本号
--@currencyVersion:货币版本号
function HttpManager:unlockHiddenMeridianBuff(id,version,currencyVersion,callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "unlock_hidden_meridian_gems", {id = id,version = version, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end

--获取师门指点相关信息
function HttpManager:getSectGuidanceInfo(familyId, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_sectGuidance_info", {familyId = familyId}, nil, callback, isNeedWait, retryType)
end

--师门指点
function HttpManager:executeSectGuidance(familyId, id, version, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "execute_sectGuidance", {familyId = familyId, id = id, version = version}, nil, callback, isNeedWait, retryType)
end

--[[
    @desc: 加入新门派时需告知服务器处理旧门派数据
    author:tanqinjian
    time:2025-03-19 17:17:43
    --@familyId:门派id
    --@currencyVersion:
	--@callback:
	--@isNeedWait:
...
function HttpManager:joinFamily(familyId, currencyVersion, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "join_family", {familyId = familyId, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end
...
--[[
    @desc: 师门指点刷新测试接口
    author:tanqinjian
    time:2025-03-24 18:21:34
    --@callback:
	--@isNeedWait:
	--@retryType: 
    @return:
]]
function HttpManager:testResetSectGuidance(callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeader(DOMAIN .. "test_reset_sect_guidance", nil, nil, callback, isNeedWait, retryType)
end

--[[
    @desc: 
    author:tanqinjian
    time:2025-05-29 18:28:08
    --@id: type 1 为奖励id 2为商品id
	--@number:
	--@type: 1、残页 2、商品
    --@currencyVersion:货币版本
	--@callback:
...
function HttpManager:exchangeYashiWelfareReward(id, number, type, currencyVersion, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "exchange_yashi_welfare_reward", {id = id, number = number, type = type, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end

--[[
    @desc:获取商人列表
    author:tanqinjian
    time:2025-06-19 20:39:26
    --@merchantId:商人id
	--@currencyVersion:货币版本
    --@isRefresh:是否手动刷新 0 不刷新 1 刷新
    --@dataVer:
	--@callback:
	--@isNeedWait:
	--@retryType: 
...
function HttpManager:getMerchantStoreList(merchantId, isRefresh, currencyVersion, dataVer, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_merchant_store_list", {merchantId = merchantId, isRefresh = isRefresh, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end

--[[
    @desc: 
    author:tanqinjian
    time:2025-06-19 21:35:12
    --@merchantId:商人id
	--@goodsList:玩家购买列表
    --@recordList:玩家出售、赎回列表，获得碎银
	--@currencyVersion:货币版本
    --@dataVer:
	--@callback:
	--@isNeedWait:
...
function HttpManager:buyMerchantGoods(merchantId, goodsList, recordList, currencyVersion, dataVer, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "buy_merchant_goods", {merchantId = merchantId, goodsList = goodsList, dataVer = dataVer, currencyVersion = currencyVersion, recordList = recordList}, nil, callback, isNeedWait, retryType)
end

--[[
    @desc: 
    author:tanqinjian
    time:2025-07-04 16:18:13
    --@currencyId:货币id
	--@currencyVersion:货币版本
	--@action:操作 add | remove 。增加或者扣除
	--@num:数量 不填默认100
	--@callback:
	--@isNeedWait:
	--@retryType: 
...
function HttpManager:testCurrency(currencyId, num, action, currencyVersion, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "test_currency", {currencyId = currencyId, dataVer = currencyVersion, num = num, action = action}, nil, callback, isNeedWait, retryType)
end

--[[
    @desc: 有版本号控制的特殊网络物品消耗
    author:tanqinjian
    time:2025-07-30 14:30:21
    --@itemId:
	--@number:
	--@currencyVersion:
	--@callback:
...
function HttpManager:consumeSpecialProps(itemId, number, currencyVersion, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "consume_special_props", {itemId = itemId, number = number, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end

--[[
    @desc: 获取充值福利信息
    author:tanqinjian
    time:2025-12-04 10:19:14
    --@callback:
	--@isNeedWait:
	--@retryType: 
    @return:
]]
function HttpManager:getRechargeBenefitsInfo(callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeader(DOMAIN .. "get_recharge_benefits_info", nil, nil, callback, isNeedWait, retryType)
end

--[[
    @desc: 领取充值福利奖励
    author:tanqinjian
    time:2025-12-04 10:20:48
    --@id:
	--@isEmail:（0：背包未满，无需邮件发送，1：背包已满，邮件发送）
	--@dataVer:
	--@currencyVersion:
	--@callback:
	--@isNeedWait:
...
function HttpManager:getRechargeBenefitsReward(id, isEmail, dataVer, currencyVersion, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_recharge_benefits_reward", {id = id, isEmail = isEmail, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end

--[[
    @desc: 获取跳转到副本商人类型活动信息
    author:tanqinjian
    time:2025-12-18 17:46:38
    --@activityId:
	--@currencyVersion:
	--@callback:
	--@isNeedWait:
...
function HttpManager:getMerchantEventInfo(activityId, currencyVersion, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_merchant_event_info", {activityId = activityId, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end


function HttpManager:getActivityChallengeMapClearTimeInfo(callback, isNeedWait, retryType)
    local callback = self:createGetResponseFunction(callback)
    self:retryGetWithHeader(DOMAIN .. "get_challengemap_task_info", nil, nil, callback, isNeedWait, retryType)
end

function HttpManager:getActivityChallengeMapClearReward(isEmail, dataVer, currencyVersion, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
    self:retryPostWithHeader(DOMAIN .. "get_challengemap_task_reward", {isEmail = isEmail, dataVer = dataVer, currencyVersion = currencyVersion}, nil, callback, isNeedWait, retryType)
end

--[[
    @desc: 获取服务器资源商品数量
    author:tanqinjian
    time:2026-05-12 15:59:57
    --@itemList:{
        {
            id = "服务器资源id", 
            type = "服务器资源类型" --目前与资源对应商品表商品itype属性一致
        }
    }
	--@dataVer:
	--@currencyVersion:
...
function HttpManager:getServerResourceCount(itemList, dataVer, currencyVersion, callback, isNeedWait, retryType)
    callback = self:createGetResponseFunction(callback)
