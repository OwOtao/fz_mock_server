...
local CacheMap = require("app.extends.CacheMap")
...
local jingmaiMap =
    {
        ...
        linggangyin = {name = "怒气翻涌", desc = "战斗中，当气血低于30%时，所有技能的内力消耗降低",
            ...
            getAdditionalAttr = function(role, target, attrName, attrValue)
                if attrName == "_neiliConsumeFactor" then
                    if role:getAttr("qiPercent") <= 0.3 then
        ...
        zhenfengyin = {name = "三花聚顶", desc = "战斗中使用主动技能时，有几率恢复一定内力",
            ...
            func = function(role, target, activeZhao)
                if role._fight and role:getAttr("neili") < role:getFinalAttr("neiliMax") then
                    if jingmaiTestSwitch or 40 >= (role._fight._currFrame % 100 + 1) and (role._fight._currFrame % 100 + 1) >= 31 then
        ...
        zhengangyin1 = {name = "招招致命", desc = "战斗中使用主动技能时，有几率提升一定攻击",
            ...
            func = function(role, target, activeZhao)
                ...
                if role._currZhaoIndex == 0 then
                    role._currZhaoIndex = 1
                end
                local zhao = inherit({}, autoZhaos[role._currZhaoIndex]) --本地战斗
...
defVars(FightRole,
    {
        _nextActiveZhaoId = {"NextActiveZhaoId"}
    })

function FightRole:create()
    local p = clone(FightRole)
    p:init()
    return p
end
...
function FightRole:init()
    ...
    -- 防御系数 .. 暂时没有数值支持 add by TangJian 2017/01/03 20:55:12
...
function FightRole:setData(data)
    ...
    --@TODO 2020-10-17 11:11:09 临时处理梦境角色和Role创建差异
    if self._data.isDreamRole == true then
        data._buffManager:serialization()
        data.emotionMgr:serialization()
        self._role = User:getRole():getDreamSystem():createDreamRoleWithData(data)
        self._role:updateActiveZhaoStatus()
    else
    ...
    --#TODO 需测试判断是否取消
    if not MapIsEmpty(self._data._shenbingCache) then
        for k,v in pairs(self._data._shenbingCache) do
            setmetatable(self._role._shenbingCache[k], getmetatable(v))
        end
    end
    ...
    -- self._role._buffManager = self._data._buffManager
...
function FightRole:reinitRole()
    if self:getReadyActiveZhaoId() ~= nil then
        self._currActiveIndex = self._currActiveIndex + 1
        self:setReadyActiveZhaoId(nil)
        
        self._fight:clearRoleActiveZhao(self)
        
        self._fight:callEventListener("unReadyActiveZhao",self:getTeamId(),self:getInTeamId())
    end
...
function FightRole:getAttr(attrName)
    ...
    if type(attrValue) == "number" then
        ...
        -- 获取状态型经脉印记加成
        local jingMaiAddAttr = self:getFightRoleJingMaiAddValue(attrName)
        attrValue = attrValue + jingMaiAddAttr
    end
...
function FightRole:getAtkSpeedFactor()
    ...
    local addValue = 0
    if weaponType ~= "拳脚" and weaponType ~= "暗器" and self._role:getEquipByName("weapon") ~= nil then   
...
function FightRole:setAttr(attrName, value)
    ...
    self:getRole():dispatchEvent("FightAttrChangeAfterEvent",{role = self,attrName = attrName})
...
function FightRole:unloadWeapon()
    if self:isPlayer() and self:getAttr("userid") == User:getUserId() then
        self._role:setEquipByName("weapon", nil)
    elseif self._role.isDreamRole == true then
        self._role:setEquipByName("weapon", nil)
    else
        self._role.equips.weapon = nil
    end
    self:reinitRole()
    ...
    if self:isForget() then
        -- add by LvBin 2019/03/20 12:11:26 目标中遗忘的情况下，不用刷新恢复按钮
        self._fight:callEventListener("unloadWeapon",self,2)
    elseif self._role.isGuYongBing == true then  --雇佣兵没有恢复
        self._fight:callEventListener("unloadWeapon",self,4)
    else
        self._fight:callEventListener("unloadWeapon",self,3)
    end

    if not self:isPlayer() then
        self._role:setFlag("兵器被打飞",1)
    end
...
-----------------------------------------------------------------------------------------------------------
--打飞兵器
function FightRole:jifeiWeapon()
	--记录丢失的兵器的子类型
...
-----------------------------------------------------------------------------------------------------------
--打断兵器
function FightRole:daduanWeapon()
	--记录丢失的兵器的子类型
    local weaponSubtype = self:getRole():getCurrSubtypeByWeapon()
    self:setBeforeUnloadWeaponIsSubType(weaponSubtype)
    ...
    if self:isPlayer() and self:getAttr("userid") == User:getUserId() then
        ...
        local weapon = self._role:getItemWithOnlyId(weaponData.id)  --普通兵器数据
        local roleWeapon = User:getRole():getItemWithOnlyId(weaponData.id)

        local weaponAttr = self._role:getOneItemByKey(weaponData.itemId)  -- 神兵数据
        ...
        weapon.wanhaodu = 0
        roleWeapon.wanhaodu = 0

        if weapon.type and weapon.type == "神兵" then    --保存神兵数据
            weaponAttr.wanhaodu = 0
            local ShenBingDuanZao = require("app.models.ShenBing.DuanZao.ShenBingDuanZao")
            ShenBingDuanZao:updateShenBingInfo(weaponAttr,User:getRole())
        end

        --@desc 修改战斗中人物装备属性
        local prepareWeapon=User:getRole():getPrepareWeapon()

        if prepareWeapon and prepareWeapon.id == weaponData.id then 
            User:getRole():setPrepareWeapon()
        else
            -- 修改人物存档中装备武器属性
...
-----------------------------------------------------------------------------------------------------------
function FightRole:changeWeapon()  
   
    -- if self:isImmobilized() or self:getAttr("tili")<20 then 
    --     return 
    -- end
    ...
    --Helper:print_lua_table(prepareWeapon)
...
function FightRole:newWeaponChange()
    ...
    local ok = false
    if self.__ChangeWeaponType == 1 then
        if currWeapon ~= nil then
            if prepareWeapon ~= nil and currWeapon.id == prepareWeapon.id then
                PopText("当前已是备用武器")
                ok = false
            elseif prepareWeapon ~= nil then
                changeWeapon = prepareWeapon
                self._role:setEquipByName("weapon", prepareWeapon)
                ok = true
            else
                ok = false
        ...
        if ok then
            ...
            elseif self.__quaoJiaoWeaponChangeState == 3 then
                ...
                self.__quaoJiaoWeaponChangeState = 2
            elseif self.__quaoJiaoWeaponChangeState == 4 then
                self.__quaoJiaoWeaponChangeState = 2
            end 
        end
    elseif self.__ChangeWeaponType == 2 then --武器切换只跟下一次切换武器有关
        if self.__replaceWeapon then
    ...
    if ok then
        ...
        if self.__ChangeWeaponType == 1 or self.__ChangeWeaponType == 2 then
            ...
            if currWeapon then 
                ...
                poptext = "切换武器！"
                local str="YEL你将NOR"..currWeaponInfo.name.."YEL收起，将"..changeWeaponInfo.name.."拿在手上。NOR"
                self._fight:callEventListener("printText",str)
            else
                poptext = "你拿出了武器！"
                local str="YEL你将NOR"..changeWeaponInfo.name.."YEL拿在手上，持械迎击。NOR"
                self._fight:callEventListener("printText",str)
            end

            if self.__ChangeWeaponType == 2 then
                poptext = "更换备用武器"
            end
...
function FightRole:setCurrAttackZhao(attackZhao)
    if self.isZhuiJia == true then
        local anims,action = Meridian:createZhaoIsAnimAndText(self,self._cType)
...
function FightRole:getAutoZhaoTiliConsume(zhao)
    ...
    -- print(self:getAttr("name") .. " tiliConsume = ", tiliConsume)
    return tiliConsume
end
...
-----------------------------------------------------------------------------------------------------------
--@desc: 获取命中部位名称和实际命中部位
--@author:LvBin
--@time:2025-08-05 15:21:45
--@return
function FightRole:getRandomAttackPart(attackSkill)
	local hitPosName, realHitPos
	if self:getAnimType() == "human" then
		hitPosName, realHitPos = attackSkill:getRandomAttackPositionPart()
	else
		hitPosName, realHitPos = FightConfig:getRandomAnimalAttackPart(self:getAnimType())
	end
	return hitPosName, realHitPos
end
-----------------------------------------------------------------------------------------------------------
-- @author TangJian
...
function FightRole:createAttackResult(__role, __target, attackSkill, attackZhao, doubleAtkSkill, doubleAtkZhao, magicNumber1, magicNumber2)
    ...
    local hitPosName, realHitPos = __target:getRandomAttackPart(attackSkill)-- 攻击部位
    ...
    if device.platform == "windows" and Game:isTesting() then
        targetDodge = targetDodge * FightConfig:testServerGMFactor("目标战斗闪躲值扩大倍数")
        targetParry = targetParry * FightConfig:testServerGMFactor("目标战斗招架值扩大倍数")
    end

    -- self:print("fightHitRate = "..tostring(fightHitRate))
    -- self:print("targetDodge = "..tostring(targetDodge))
    -- self:print("targetParry = "..tostring(targetParry))
    ...
    LogSystem:log("旧版战斗：","攻击者 = ",fightRole:getName()," |受击者 = ", target:getName()," |主手武学id = ",attackSkill.id," |主手招式id = ",attackZhao.id)
    
    LogSystem:log("旧版战斗：","招式原始结果(1 命中，2 招架，3 闪躲)：",hitType," |攻击者atkSpeedFactor值：",__role:getAttr("atkSpeedFactor")," |攻击者hitRateFactor值：",__role:getAttr("hitRateFactor")," |招式命中值：",fightHitRate," |受击者dodgeRateFactor值：",__target:getAttr("dodgeRateFactor")," |目标闪躲值：",targetDodge," |受击者parryRateFactor值：",__target:getAttr("parryRateFactor")," |目标招架值：",targetParry," |随机值1：",magicNumber1," |随机值2：",magicNumber2," |闪避率：",trueDodgeRate," |招架率：",trueParryRate)

    if hitType == HIT_TYPE_HIT then
        local rate =  fightRole:getFinalAttr("zhaoHitReduceRate")
        if rate > 0 and math.random(1,100) < rate then
            local list = {HIT_TYPE_DODGE,HIT_TYPE_PARRY}
            hitType = list[math.random(1,2)]
        end
    else
        local isHit = fightRole:getFinalAttr("zhaoIsHit")
        if isHit ~= 0 then
            hitType = HIT_TYPE_HIT
        end
    end
    ...
    if doubleAtkSkill then
        ...
        doubleAtkZhao.atkSkillId = doubleAtkSkill.id

        --@desc 自创伤害修正系数
        local selfCreatedSkillFactor = 1
        LogSystem:log("旧版战斗：","副手武学id = ",doubleAtkSkill.id," |副手招式id = ",doubleAtkZhao.id)
        LogSystem:log("旧版战斗：","主手武学被动招式消耗的体力 = ",self:getAutoZhaoTiliConsume(attackZhao))
        LogSystem:log("旧版战斗：","副手武学被动招式消耗的体力 = ",self:getAutoZhaoTiliConsume(doubleAtkZhao))
        ...
        if doubleAtkSkill.type == SKILL_TYPE_SELFCREATE then
            ...
            LogSystem:log("旧版战斗：","自创副手基础伤害比例 = ",baseParam)
            LogSystem:log("旧版战斗：","自创副手体力修正伤害比例 = ",tiliParam)
            LogSystem:log("旧版战斗：","自创伤害修正系数 =",selfCreatedSkillFactor)
        end

        LogSystem:log("旧版战斗：","主手气血伤害 = ",atk * qiAtkFactor," |主手气血上限伤害 = ", qiMaxAtk * qiMaxAtkFactor)
        LogSystem:log("旧版战斗：","副手气血伤害 = ",fightRole:getQiAtk(doubleAtkZhao, target, realHitPos,__role) * selfCreatedSkillFactor * qiAtkFactor," |副手气血上限伤害 = ", fightRole:getQiMaxAtk(doubleAtkZhao, target, realHitPos,__role) * selfCreatedSkillFactor * qiMaxAtkFactor)
        LogSystem:log("旧版战斗：","攻击部位 = ",hitPosName)
        qiMaxAtk = (qiMaxAtk + fightRole:getQiMaxAtk(doubleAtkZhao, target, realHitPos,__role) * selfCreatedSkillFactor) * qiMaxAtkFactor
        atk = (atk + fightRole:getQiAtk(doubleAtkZhao, target, realHitPos,__role) * selfCreatedSkillFactor) * qiAtkFactor
        neiliConsume = 2 * neiliConsume * jiaLiConsumeFactor
        LogSystem:log("旧版战斗：","主副合计手气血伤害 = ",atk," |主副合计手气血上限伤害 = ", qiMaxAtk)
    end

    -- 攻击力取整
    atk = atk + qiMaxAtk
    atk = math.floor(atk)
    LogSystem:log("旧版战斗：","攻击 = ",atk, " |平均气血伤害avgqiatk = ",fightRole:getAvgQiAtk(target))

    -- 经脉追加的招式重新根据武功等级计算伤害和内力消耗
    if __role.isZhuiJia then
        local atkSkillId = fightRole:getPrepareSkill(__role._cType)
        local jibenLv = fightRole:getSkillLv("jiben"..__role._cType)
        local useSkillLv = atkSkillId == "jiben"..__role._cType and 0 or fightRole:getSkillLv(atkSkillId)
        ...
        neiliConsume = 0
        LogSystem:log("旧版战斗：","经脉追加后攻击 = ",atk)
    end
    
    return hitType, atk, qiMaxAtk, neiliConsume, hitPosName
end
...
function FightRole:createAutoZhao(__target, magicNumber1, magicNumber2)
    ...
    local hitType, atk, qiMaxAtk, neiliConsume, hitPosName = self:createAttackResult(self, __target, atkSkill, atkZhao, doubleAtkSkill, doubleAtkZhao, magicNumber1, magicNumber2)
    ...
    local attackZhaoTable =
        {
            ...
            befFrame = befFrame, --前摇
            atkFrame = atkFrame, --攻击
            aftFrame = aftFrame, --后摇

            hitType = hitType, --攻击结果类型
    ...
    -- print( "skill id= " .. atkSkill.id .. " zhaoid=" .. atkZhao.id )
...
function FightRole:createJingMaiEffectAutoZhao(__target, magicNumber1, magicNumber2, cType)
    ...
    local hitType, atk, qiMaxAtk, neiliConsume, hitPosName = self:createAttackResult(self, __target, atkSkill, atkZhao, nil, nil, magicNumber1, magicNumber2)
...
function FightRole:createAutoZhaos(target, autoZhaoCount)
    ...
    autoZhaoCount = Helper:getRange(autoZhaoCount, 0, 100)
...
function FightRole:getActiveZhaoState(activeZhaoId)
    return assert(self._currActiveZhaoStateMap[activeZhaoId], "找不到主动技能 " .. activeZhaoId)
end

--查找当前招式
function FightRole:getActiveZhaoByActiveZhaoId(activeZhaoId)
...
function FightRole:addEffect(effect)
    print("FightRole:addEffect(effect)" .. effect:getId())
    local isExist = false
...
-----------------------------------------------------------------------------------------------------------
-- @author GaoHanZheng
-- @time 2018/01/21 18:39:13
-- @desc 是否反击
function FightRole:getReFightValue()
    for k, effect in pairs(self._effectMap) do
        print("===============================:",effect:getType())
        if effect:getType() == "招架反击" then
            return true
        end
    end
    return false
    ...
    if  self:getRole():getSkill("zhengaoxuanjing") ~= nil  then
        ...
        if isDie == true and ran_num <= trigger_num and self._isRevive ~= true then
            ...
            for i,v in ipairs(list) do
                local effect = Skill:getSkillEffect(v):clone()
                effect:setOwner(self)
                effect:setObject(self)
...
function FightRole:getPreparedActiveZhaoArray()
    ...
    elseif self._role.isNpc or self._role.isGuYongBing or self._role._isZhang or self._role.isInLunJian then
        return self._role:getPreparedActiveZhaoArray()
    else
...
function FightRole:getActiveZhaoMap()
    ...
    for i, activeZhaoId in pairs(prepareZhaoList) do
        preparedActiveZhaoMap[activeZhaoId] = Skill:getActiveZhao(activeZhaoId):clone()
    end
...
function FightRole:jingMaiUpdate()
    --经脉不存在  例如：论剑中角色对象 清空了经脉
    if not self._fightRoleJingMai then
        return
    end
...
--@desc: 获取角色神兵特效信息
function FightRole:getRoleWeaponEffect()
    return self._WeaponEffectArray
end

--@desc:刷新战斗角色buff
function FightRole:updateFightRoleBuff()
    local fight = self._fight
    if not fight then
        return
    end
    ...
    if target then
        self._role:updateRoleBuff(target._role)
    end
end
...
function FightRole:cheackHaveSpecialMeridianEffect()
	...
	for i,v in ipairs(specialMeridianList) do
		if role:isHaveImprintingId(v.id) and (v.value >= math.random( 1,100) or DEBUG_MODE == 1) then
	...
	if MapIsEmpty(meridianList) then
        ...
        for i,v in ipairs(specialMeridianList) do
            ...
            if randWeight then
                ...
                if randWeight > 0 and randWeight > math.random( 1,100) then
                    table.insert( meridianList, {id = v.id, effect = v.effect, name = v.name})
                end
            end
        end
        ...
        meridian = meridianList[math.random(1,#meridianList)]
...
--@desc: 获取伤害力系数
--@author:Seven_L
--@time:2020-07-02 18:14:50
--@target: 
function FightRole:getDamageFinalFactor(target)
    if target == nil then
        return 1
    end

    if self:getAttr("qiAtkFactor") == 1 and target:getAttr("defRateFactor") == 1  then
        return 1
    end

    local value = 1 + self:getAttr("qiAtkFactor") - target:getAttr("defRateFactor")
    local finalFactor = Helper:getRange(value, 0.2, 5)
    if PRINT_MODE == 1 then
        print("气血攻击系数————————————>"..self:getAttr("qiAtkFactor"))
        print("气血防御系数————————————>"..target:getAttr("defRateFactor"))
        print("气血最终伤害系数————————————>"..finalFactor)
    end
...
--@desc 气血上限伤害系数
function FightRole:getQiMaxDamageFactor()
    return self:getAttr("qiMaxAtkFactor")
end

-- 梦境中切磋和决斗的（逃跑）功能调整：
-- 梦境战斗中点击逃跑按钮时，根据梦境主角所属门派，读关联表格，字段（内力消耗值;kcneili）填值，判断主角内力是否足够：
-- 内力不足，弹出文本：内力匮乏，无法脱逃！ 按钮进入冷却状态（CD1秒，和主动技能一样的表现）；
-- 内力足够，读关联表格，字段（逃跑概率;probabilitys）填值，判断是否成功：
-- 逃跑成功，扣除内力（字段：内力消耗值;kcneili），和原有机制一样，战斗结束，气血和伤势和内力保留；
-- 逃跑失败，弹出文本：对方穷追不舍，脱逃失败！ 按钮进入冷却状态（CD1秒，和主动技能一样的表现）；

function FightRole:checkRunAwayConsumeNeili()
    return self._role:getAttr("neili") >= Helper:getDef(self._role.kcneili,0)
end

function FightRole:runAway()
    if self._role.isDreamRole == true then
        local runRole = self._role
        local isRun,msg = false,""
        print("------梦境逃跑输出--------")

        if runRole.runprobability == nil then
            return true
        end

        if self:checkRunAwayConsumeNeili() then
            local runprobability = math.random(1,100)
            if runRole.runprobability > runprobability then
                isRun = true
                msg = "施展轻功溜之大吉，扣除些许内力！"
            else
                msg = "脱逃失败，内力亦有所损耗！"
                self.runAwayCD = runRole.times
            end

            local costNeili = Helper:getDef(runRole.kcneili,0)
            if costNeili ~= 0 then
                runRole:addAttr("neili", - costNeili)
            end
            print("------当前逃跑概率：",runRole.runprobability," 随机概率：",runprobability)
        else
            msg = "内力匮乏，无法脱逃！"
            self.runAwayCD = runRole.times
        end
...
function FightRole:getRunAwayCd()
    return self.runAwayCD
end

function FightRole:chearRunAwayCd()
    self.runAwayCD = nil
end
...
function FightRole:getFistFootEffects()
    return self._role:getFistFootEffects()
end

function FightRole:recordFistFootEffect(effectId)
    if not self.__fistFootEffects then
        self.__fistFootEffects = {}
    end

    if self.__fistFootEffects[effectId] then
        return
    end

    if effectId then
        self.__fistFootEffects[effectId] = true
    else
...
function FightRole:getJqdamage()
    ...
    if weapon == nil or weapontype == "拳脚" then
        if not self.__jqdamage then
            local fistFootTechniques_1 = self._role:getPrepFistFootTechniques()
            local fistFootTechniques_2 = self._role:getStandByFistFootTechniques()

            for i, t in ipairs(fistFootTechniques_1) do
                local t = t
                jqdamage = jqdamage + t:getJqdamage()
            end

            for i, t in ipairs(fistFootTechniques_2) do
                local t = t
                jqdamage = jqdamage + t:getJqdamage()
            end

            self.__jqdamage = jqdamage
        else
            jqdamage = self.__jqdamage
        end
    end
...
function FightRole:getPrepJqdamage()
    if self.__prepJqdamage then
        return self.__prepJqdamage
    end

    local jqdamage = 0

    jqdamage = self._role:getPrepJqdamage()

    self.__prepJqdamage = jqdamage
...
function FightRole:getStandByJqdamage()
    if self.__standByJqdamage then
        return self.__standByJqdamage
    end

    local jqdamage = 0

    jqdamage = self._role:getStandByJqdamage()

    self.__standByJqdamage = jqdamage
...
function FightRole:addRoleDefaultEffect()
    --论剑不触发
    if self._role.isInLunJian == true then
        return 
    end
...
function FightRole:reversibleDisarmWeaponEnd()
    ...
    if disarmWeapon then
        local disarmWeaponAttr = self._role:getItemWithOnlyId(disarmWeapon.id)  --普通兵器数据

        if disarmWeaponAttr.wanhaodu and disarmWeaponAttr.wanhaodu == 0 then
