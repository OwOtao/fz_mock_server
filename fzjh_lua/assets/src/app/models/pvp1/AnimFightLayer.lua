...
function Player:init()
    ...
    do
        self._hudunAnim = Resource:getSkAnim("hudun")
        self:addChild(self._hudunAnim, 10)
        self._hudunAnim:setVisible(true)
        self._hudunAnim:setOpacity(0)
        
        self._hudunAnim.isEnable = true
        self._hudunAnim.isVisible = false
        
        self._hudunAnim:setAttachment("bian_attack7_0", "")
    -- self._hudunAnim:playAnim("fanghu-1", true)
    -- self._hudunAnim:playAnim("bai", true)
    end
...
-- -- 角色头顶的释放招式效果
-- self.activeZhaoReadyItem = RoleStatePanelActiveZhaoReadyItem:create()
-- self:addChild(self.activeZhaoReadyItem)
-- self.activeZhaoReadyItem:setAnchorPoint(cc.p(0.5, 0.5))
-- self.activeZhaoReadyItem:move(0, 180)
--解析角色技能，创建技能对象
-- self:schedule(function()
-- 	self.activeZhaoReadyItem:show()
-- end, 2)
end
...
function Player:setShieldAnimEnabled(b)
    ...
    if b == true then
        if self._hudunAnim.isVisible == true then
            self._hudunAnim:fadeIn()
            -- self._hudunAnim:setVisible(true)
        else
            self._hudunAnim:fadeOut()
...
function Player:setShieldAnimColor(colorName)
    local animName = switch(colorName,
        {
            ["白"] = "bai",
            ["金"] = "jin",
            ["蓝"] = "lan",
    ...
    self._hudunAnim:playAnim(animName, true)
end
...
-- @author TangJian
-- @desc
function AnimFightLayer:createInPanel(parent)
    local p = AnimFightLayer:new()
    p:init(parent)
    return p
end

function AnimFightLayer:init(parent)
...
-- @author TangJian
-- @desc 初始化背景层
function AnimFightLayer:initBackGroundLayer()
    self._backGroundLayer = cc.Layer:create()
    self:addChild(self._backGroundLayer)
    
    self._backA = ccui.ImageView:create("fightback.png")
    self._backB = ccui.ImageView:create("fightback.png")
    
    self._backGroundLayer:addChild(self._backA)
    self._backGroundLayer:addChild(self._backB)
    
    self._backA:setPositionY(self._size.height / 2)
    self._backB:setPositionY(self._size.height / 2)
    
    self._backA:setAnchorPoint(cc.p(0, 0.5))
    self._backB:setAnchorPoint(cc.p(0, 0.5))
end
