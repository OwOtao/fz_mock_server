local task =
{
	...
	-- 显示
	buttonA = "taskButton12a",
	buttonB = "taskButton12b",

	-- 数据
	type = "挂机任务", -- 任务类型
	name = "护送军机", -- 任务名称

	minExp = 15000000, -- 最小经验值
	maxExp = 30000000, -- 最大经验值
	...
	guajiReward = -- 挂机奖励
	{
		exp = function(sklv, fy,Lv)	-- 实际值
			-- return 13224*(2^(sklv/665))*math.min(1.2,math.random(80,108+math.floor(0.5*fy))/100) / 3600
			return 13224*(6*sklv+655)/(Lv+sklv+982)*(2*fy+115)/(fy+130)*math.random(85,115)/100 / 3600
		end,
		pot = function(sklv, fy)
			return 6612*(2^(sklv/665))*math.min(1.2,math.random(80,108+math.floor(0.5*fy))/100) / 3600
		end,
		money = function(sklv, fy)
			return 6000/3600*math.random(80,108+math.floor(0.5*fy))/100
		end,
		avgExp = function(sklv, fy,Lv)	-- 平均值
			return 13224*(6*sklv+655)/(Lv+sklv+982)*(2*fy+115)/(fy+130)/3600
		end,
		avgPot = function(sklv, fy)
			return 6612*(2^(sklv/665))*math.min(1.2,0.9375+fy/400) / 3600
		end,
		avgMoney = function(sklv, fy)
			return 6000*math.min(1.2,0.9375+fy/400) / 3600
		end,
		maxExp = function(sklv, fy)	-- 最大值
			return 30415*(2*fy+115)/(fy+130)/ 3600
		end,
		maxPot = function(sklv, fy)
			return 13250*math.min(1.2,0.9375+fy/400) / 3600
		end,
