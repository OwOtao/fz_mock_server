...
local JumpBackTween = require("app.FightSystem.Veiws.ViewCommonModel.ViewTween.JumpBackTween")
...
local CharacterJumpBackViewEvent = {}

function CharacterJumpBackViewEvent:create(...)
    return CharacterJumpBackViewEvent.new():__init(...)
end

function CharacterJumpBackViewEvent:__init(jumperId, jumpDuration)
...
function CharacterJumpBackViewEvent:doViewEvent(mainView)
...
function CharacterJumpBackViewEvent:__jumpInit()
    ...
    local x, y = self.__jumperViewCharacter:getOriginStartPosition()
...
function CharacterJumpBackViewEvent:__jumperJump()
    local jumpTween =
        JumpBackTween:create(
        function()
    ...
    self.__mainView:playCharacterAnim(self.__jumperViewCharacter:getId(), self.__jumperViewCharacter:getJumpBackAnimName(), false)
    ...
    local jumpHTween =
        ...
        function(next)
            self.__jumperViewCharacter:setPositionH(next)
...
function CharacterJumpBackViewEvent:__blockerJump()
    ...
    self.__mainView:walkViewCharacters(
        function(otherCharacter)
            ...
            if otherCharacter == self.__jumperViewCharacter then
                return false
            end
    ...
    local originX, originY = blockerViewCharacter:getOriginStartPosition()
    ...
    self.__mainView:playCharacterAnim(blockerViewCharacter:getId(), blockerViewCharacter:getJumpBackAnimName(), false)

    self.__mainView:doUITween(blockJumpTween)
    ...
    local jumpHTween =
        ...
        function(next)
            blockerViewCharacter:setPositionH(next)
...
return newClass("CharacterJumpBackViewEvent", {IViewEvent}, CharacterJumpBackViewEvent)
0000