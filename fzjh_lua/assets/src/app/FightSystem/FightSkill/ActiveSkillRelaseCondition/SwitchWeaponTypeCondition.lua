...
local SwitchWeaponTypeCondition = {
    __type = 2
}

function SwitchWeaponTypeCondition:create(placeholder, logicalSymbol, placeholder2, condValue, failureTextId)
    local p = SwitchWeaponTypeCondition.new()
    p:__init(placeholder, logicalSymbol, placeholder2, condValue, failureTextId)
    return p
end

function SwitchWeaponTypeCondition:__init(placeholder, logicalSymbol, placeholder2, condValue, failureTextId)
...
function SwitchWeaponTypeCondition:canRelease(f_character)
    local targetWeapon = f_character:getSwitchTargetWeapon()

    local isMatch = false
    if self.__logicalSymbol == "等于" then
        if targetWeapon:getFirstType() == self.__condValue then
            isMatch = true
        else
            isMatch = false
        end
...
return newClass("SwitchWeaponTypeCondition", {IActiveSkillReleaseCondition}, SwitchWeaponTypeCondition)
00