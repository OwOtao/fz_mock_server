...
function BuffEffect211:create()
    return BuffEffect211.new():__init()
end

function BuffEffect211:__init()
    self.__isInit = false
...
function BuffEffect211:updateEffectValue()
...
function BuffEffect211:makeEffectOnAdd()
    ...
    if self.__type == 1 then
        self.__attrName = "autoNeiliCorrectionFactor"
    elseif self.__type == 2 then
        self.__attrName = "activeNeiliConstCorrectionFactor"
    else
        error("效果类型 211 未知影响招式类型：" .. tostring(self.__type))
    end

    self.__onlyId = self.__buff:getBuffOwner():addBuffMaxValue(self.__attrName, self.__addValue, self.__basicEffect:getEffectID())
end

function BuffEffect211:makeEffectOnRemove()
...
function BuffEffect211:makeEffectOnTransfer(buffEffect)
end

return newClass("BuffEffect211", {ABuffEffect}, BuffEffect211)
0000000000000