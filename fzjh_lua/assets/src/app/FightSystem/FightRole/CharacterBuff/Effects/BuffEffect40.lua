--[[
    ...
    desc: 
        **效果类型ID=40，造成被动招式攻击气血伤害比例N%影响目标当前属性**

        * 效果功能执行：角色被动招式实际造成的气血伤害X点，按比例N%，影响指定目标的指定当前属性M点。
        ...
        * T1=角色头顶弹字提示(受击帧)（影响目标=def），T6=攻击者头顶弹字提示(受击帧)（影响目标=self），弹字时间与受击帧时间一致。
            * 效果值M=0，弹字提示数字会显示0
]]
...
function BuffEffect40:create()
    return BuffEffect40.new():__init()
end

function BuffEffect40:__init()
    self.__isInit = false
...
function BuffEffect40:updateEffectValue()
...
function BuffEffect40:makeEffectOnAdd()
...
function BuffEffect40:__onAutoZhaoAttack(context, oneAttackHitResult)
    ...
    if target ~= self.__buff:getBuffOwner():getTarget() then
        error(" BuffEffect40:__onAutoZhaoAttack 当前上下文攻击目标与该效果持有角色的攻击目标不一致，检查代码流程")
    end
...
function BuffEffect40:makeEffectOnRemove()
end
...
function BuffEffect40:makeEffectOnTransfer(buffEffect)
end

return newClass("BuffEffect40", {ABuffEffect}, BuffEffect40)
0000