...
function ActiveZhaoHurt:getHurtValue()
    ...
    local activeAttackQiHurtValue = 1
    if self:getChangeAttrType() == 1 then
        activeAttackQiHurtValue = FightFormula:calActiveAttackQiHurtCorrectionFactor(self.__character:getAttr("qiActiveAtkFactor"), self.__character:getTarget():getAttr("qiActiveDefFactor"))
    end
    ...
    if tostring(self.__zhaoHurtRes.activeZhaoAtkDamageClass) ~= "0" then
        skillDamageAttrCorrectionFactor = FightFormula:calSkillDamageAttrCorrectionFactor(self.__character, self.__character:getTarget(), tostring(self.__zhaoHurtRes.activeZhaoAtkDamageClass))
    end

    local activeHurtValue = FightFormula:calActiveAttackHurtValue(autoAvgAtk, hurtDegreeValue, self:getHurtGrow(), self:getHurtBase(), activeAttackQiHurtValue, skillDamageAttrCorrectionFactor)
