...
local ActiveSkillBanFilter = {}

function ActiveSkillBanFilter:create()
    return ActiveSkillBanFilter.new()
end
...
--@return void
function ActiveSkillBanFilter:doFilter(operationCommand, operationCommandResponse, filterChain)
    ...
    local isBan, tip = commander:activeSkillIsBan(activeSkill:getActiveType())

    if isBan == true then
        operationCommandResponse:setResult(false)
        operationCommandResponse:setResultPopText(tip)
        return
    end
...
return newClass("ActiveSkillBanFilter", {AOperationCommandFilter}, ActiveSkillBanFilter)
0000000000000000